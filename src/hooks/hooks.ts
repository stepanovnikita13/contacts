...
export const useLongPress = <T>(
	...
	const start = useCallback(
		(e: React.MouseEvent<T> | React.TouchEvent<T>) => {
			...
			timeout.current = setTimeout(() => {
				if (onLongPress) onLongPress(clonedEvent);
	...
	const clear = useCallback((
		...
		shouldTriggerClick && !longPressTriggered && !touchMoveTriggered && onClick?.(e);
	...
	return {
		...
		onTouchMove: (e: React.TouchEvent<T>) => { clear(e, false); setTouchMoveTriggered(true) },
	};
};